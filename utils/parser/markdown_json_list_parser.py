...
from utils.react_tool.toolkit import (
    extract_json_block,
...
def fix_json_format(json_str: str) -> str:
    ...
    json_str = re.sub(
        ...
        if not (
            m.group(5).lower() in ["true", "false", "null"]
            or re.match(r"^\d+\.?\d*$", m.group(5))
        )
...
def extract_clean_json(raw_output: str) -> Union[List[Dict[str, Any]], None]:
    ...
    raw_json_str = (
        extract_res.content
        if extract_res.status == ServiceExecStatus.SUCCESS
        else raw_output
    )
