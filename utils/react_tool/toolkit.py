    ...
    try:
        ...
        if not isinstance(parsed_result, list):
            return ServiceResponse(
                ...
                content={
                    "error": "解析结果不是列表类型",
                    "actual_type": type(parsed_result).__name__,
                },
            )
        ...
        for i, item in enumerate(parsed_result):
            ...
            try:
                ...
                if not (0 <= item["p"] <= 1):
                    ...
                    logger.warning(
                        f"项目 {i} 的置信度值 {original_p} 超出有效范围，已调整为 {item['p']}"
                    )
        ...
        return ServiceResponse(
            status=ServiceExecStatus.SUCCESS, content=validated_items
        )
    except json.JSONDecodeError as e:
        logger.error(f"JSON解析错误: {e}")
        logger.error(f"原始JSON字符串: {repr(raw_json_str)}")
        logger.error(
            f"清洗后的JSON字符串: {repr(cleaned_json) if 'cleaned_json' in locals() else '未生成'}"
        )
        return ServiceResponse(
            status=ServiceExecStatus.ERROR,
...
def extract_json_block(text: str) -> ServiceResponse:
    ...
    try:
        ...
        if not text or not text.strip():
            return ServiceResponse(
                status=ServiceExecStatus.ERROR, content={"error": "输入文本为空"}
            )
        ...
        if matches:
            ...
            return ServiceResponse(
                status=ServiceExecStatus.SUCCESS, content=extracted_json
            )
        else:
            # 如果没有找到代码块，尝试直接解析整个文本
            logger.debug("未找到JSON代码块，将尝试直接解析整个文本")
            return ServiceResponse(status=ServiceExecStatus.SUCCESS, content=text)
    except Exception as e:
        logger.error(f"提取JSON代码块时出错: {e}", exc_info=True)
        return ServiceResponse(
            status=ServiceExecStatus.ERROR, content={"error": str(e), "raw_input": text}
        )
...
def repair_broken_json(text: str) -> ServiceResponse:
    ...
    try:
        if not text or not text.strip():
            return ServiceResponse(
                status=ServiceExecStatus.ERROR, content={"error": "输入为空"}
            )
        ...
        return ServiceResponse(status=ServiceExecStatus.SUCCESS, content=parsed)

    except Exception as e:
        return ServiceResponse(
            status=ServiceExecStatus.ERROR,
            content={"error": f"JSON修复失败: {str(e)}", "raw_input": text},
        )
