...
class TCMZhengmingItem(BaseModel):
    ...
    zhengming: str = Field(..., min_length=1, description="中医证名")
    description: str = Field(..., min_length=1, description="病理分析描述")
    probability: float = Field(..., ge=0, le=1, description="诊断概率 0-1")
    therapy: str = Field(..., min_length=1, description="治法")


class HerbalPrescriptionItem(BaseModel):
    """中药方剂项"""

    zhengming: str = Field(..., min_length=1, description="对应的中医证名")
    prescription_type: str = Field(..., min_length=1, description="方剂类型")
    prescription_name: str = Field(..., min_length=1, description="方剂名称")
    composition: str = Field(..., min_length=1, description="方剂组成")
    usage: str = Field(..., min_length=1, description="用法说明")
