...
class Settings(BaseSettings):
    ...
    CORS_EXPOSE_HEADERS: List[str] = Field(
        default=[], description="Headers exposed to the browser"
    )
    CORS_MAX_AGE: int = Field(
        default=600, description="Maximum age for CORS preflight cache (in seconds)"
    )
    ...
    WORKSPACE_ROOT: str = Field(
        default="/tmp/vet-ai-workspace",
        description="WebSocket管理器的内部工作空间根目录",
    )
    EXTERNAL_WORKSPACE_ROOT: str = Field(
        default="/tmp/vet-ai-external-workspace",
        description="WebSocket管理器的外部工作空间根目录",
    )
    ...
    ENABLE_DOCS: bool = Field(
        default=True, description="Enable API documentation (Swagger UI, ReDoc)"
    )
    ...
    TASK_QUEUE_MAX_CONCURRENT: int = Field(
        default=5, description="Maximum concurrent tasks for the queue"
    )
    TASK_TIMEOUT: int = Field(default=600, description="Task timeout in seconds")
    TASK_RESULT_EXPIRE: int = Field(
        default=3600, description="Task result cache time in seconds"
    )

    # Legacy database settings (kept for compatibility but not used)
    DATABASE_ECHO: bool = Field(
        default=False, description="Legacy setting, not used with MongoDB"
    )
