...
async def create_herb_diagnosis(
    ...
    try:
        ...
        logger.debug(f"zhengming type: {type(zhengming)}, value: {zhengming if not isinstance(zhengming, list) or len(zhengming) < 5 else f'[list with {len(zhengming)} items]'}")
        logger.debug(f"prescriptions type: {type(prescriptions)}, count: {len(prescriptions) if isinstance(prescriptions, list) else 'N/A'}")
        logger.debug(f"nursing type: {type(nursing)}, count: {len(nursing) if isinstance(nursing, list) else 'N/A'}")
        ...
        logger.info(f"中医诊断完成，返回 {len(zhengming)} 个证型，{len(prescriptions)} 个方剂，{len(nursing)} 条护理建议")

        # 提取护理建议（按类别分组）
        base_nursing = next((n.content for n in nursing if getattr(n, "category", "") == "base"), "饮食清淡易消化，保持环境温暖，适当运动")
        continue_nursing = next((n.content for n in nursing if getattr(n, "category", "") == "continue"), "观察症状变化，监测精神状态")
        suggest_nursing = next((n.content for n in nursing if getattr(n, "category", "") == "suggest"), "持续呕吐腹泻或高热应立即就医")
        ...
        try:
            for idx, z in enumerate(zhengming):
                ...
                for p in prescriptions:
                    ...
                    if p_zhengming and (p_zhengming == zhengming_name or zhengming_name in p_zhengming or p_zhengming in zhengming_name):
                ...
                for idx, p in enumerate(related_prescriptions):
                    logger.debug(f"方剂 {idx} type={type(p)}, zhengming={getattr(p, 'zhengming', '')}, prescription_type={getattr(p, 'prescription_type', '')}")
                    if hasattr(p, 'dict'):
                        p_dict = p.dict()
                ...
                logger.debug(f"p_dict_list 包含 {len(p_dict_list)} 个方剂: {p_dict_list}")

                # 分类方剂（确保是字典类型）
                # 使用更严格的检查确保找到的是dict对象
                base_presc = next((p for p in p_dict_list if isinstance(p, dict) and p.get("prescription_type") == "基础方"), {})
                continue_presc = next((p for p in p_dict_list if isinstance(p, dict) and p.get("prescription_type") == "加减方"), {})
                suggest_presc = next((p for p in p_dict_list if isinstance(p, dict) and p.get("prescription_type") == "急救方"), {})

                # 额外安全检查：确保结果确实是dict
                if not isinstance(base_presc, dict):
                    logger.warning(f"base_presc 不是dict: {type(base_presc)}, 值: {base_presc}")
                    base_presc = {}
                if not isinstance(continue_presc, dict):
                    logger.warning(f"continue_presc 不是dict: {type(continue_presc)}, 值: {continue_presc}")
                    continue_presc = {}
                if not isinstance(suggest_presc, dict):
                    logger.warning(f"suggest_presc 不是dict: {type(suggest_presc)}, 值: {suggest_presc}")
                    suggest_presc = {}

                logger.debug(f"分类后的方剂 - base: {base_presc}, continue: {continue_presc}, suggest: {suggest_presc}")
                ...
                formatted_result.append({
                    ...
                    "base": str(base_nursing) if base_nursing else "",  # 确保是字符串
                    "continue": str(continue_nursing) if continue_nursing else "",
                    "suggest": str(suggest_nursing) if suggest_nursing else "",
                    "base_prescription": base_presc.get("prescription_name", "") if isinstance(base_presc, dict) else "",
                    "base_prescription_usage": base_presc.get("usage", "") if isinstance(base_presc, dict) else "",
                    "continue_prescription": continue_presc.get("prescription_name", "") if isinstance(continue_presc, dict) else "",
                    "continue_prescription_usage": continue_presc.get("usage", "") if isinstance(continue_presc, dict) else "",
                    "suggest_prescription": suggest_presc.get("prescription_name", "") if isinstance(suggest_presc, dict) else "",
                    "suggest_prescription_usage": suggest_presc.get("usage", "") if isinstance(suggest_presc, dict) else "",
                })
